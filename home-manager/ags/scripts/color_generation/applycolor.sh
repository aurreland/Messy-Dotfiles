...
apply_kitty() {
    # Check if scripts/templates/terminal/kitty/colors.conf exists
    if [! -f "scripts/templates/terminal/kitty/colors.conf"]; then
        echo "Template file not found for Kitty colors. Skipping that."
        return
    fi
    # Copy template
    mkdir -p "$CACHE_DIR"/user/generated/terminal/kitty
    cp scripts/templates/terminal/kitty/colors.conf "$CACHE_DIR"/user/generated/terminal/kitty/colors.conf
    # Apply colors
    for i in "${!colorlist[@]}"; do
        sed -i "s/{{ ${colorlist[$i]} }}/${colorvalues[$i]#\#}/g" "$CACHE_DIR"/user/generated/terminal/kitty/colors.conf
    done
    sed -i "s/{{ \$alpha }}/$term_alpha/" "$CACHE_DIR"/user/generated/terminal/kitty/colors.conf

    cp "$CACHE_DIR"/user/generated/terminal/kitty/colors.conf "$XDG_CONFIG_HOME"/kitty/colors.conf
...
apply_heroic() {
    # Check if scripts/templates/heroic/material.css exists
    if [ ! -f "scripts/templates/heroic/material.css" ]; then
        echo "Template file not found for Heroic css. Skipping that."
        return
    fi
    # Copy template
    mkdir -p "$CACHE_DIR"/user/generated/heroic
    cp "scripts/templates/heroic/material.css" "$CACHE_DIR"/user/generated/heroic/material.css
    # Apply colors
    for i in "${!colorlist[@]}"; do
        sed -i "s/{{ ${colorlist[$i]} }}/${colorvalues[$i]#\#}/g" "$CACHE_DIR"/user/generated/heroic/material.css
    done

    mkdir -p "$XDG_CONFIG_HOME"/heroic/theme
    cp "$CACHE_DIR"/user/generated/heroic/material.css "$XDG_CONFIG_HOME"/heroic/theme/material.css
}
