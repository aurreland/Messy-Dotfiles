...
export default (overviewMonitor = 0) => {
  ...
  const Window = (
    ...
    const appIcon = iconExists(iconName)
      ...
      : MaterialIcon("terminal", "gigantic", {
          css: `font-size: ${
            (Math.min(w, h) * userOptions.overview.scale) / 2.5
          }px`,
        });
    return Widget.Button({
      ...
      css: `
                ...
                margin-right: -${Math.round(
                  (x + w) * userOptions.overview.scale
                )}px;
                margin-bottom: -${Math.round(
                  (y + h) * userOptions.overview.scale
                )}px;
            `,
      onClicked: (self) => {
  ...
  const Workspace = (index) => {
      ...
      attribute: {
        put: (widget, x, y) => {
          ...
          const newCss = `
                        ...
                        margin-right: -${Math.round(
                          x + widget.attribute.w * userOptions.overview.scale
                        )}px;
                        margin-bottom: -${Math.round(
                          y + widget.attribute.h * userOptions.overview.scale
                        )}px;
                    `;
          widget.css = newCss;
      ...
      Widget.Label({
        ...
        css: `
                margin: ${
                  ...
                  userOptions.overview.scale *
                  userOptions.overview.wsNumMarginScale
                }px;
                font-size: ${
                  monitors[overviewMonitor].height *
                  userOptions.overview.scale *
                  userOptions.overview.wsNumScale
                }px;
            `,
        setup: (self) =>
    ...
    const widget = Widget.Box({
      ...
      css: `
                min-width: ${
                  monitors[overviewMonitor].width * userOptions.overview.scale
                }px;
                min-height: ${
                  monitors[overviewMonitor].height * userOptions.overview.scale
                }px;
            `,
      children: [
        Widget.EventBox({
